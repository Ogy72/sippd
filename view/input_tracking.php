...
<html lang="en">
  <head>
    ...
    <?php
        ...
        $tracking->username = $_COOKIE["user"];
        $nama = $_COOKIE['nama'];
        $level = $_COOKIE['level'];
            ...
            <div class="col-12">
              ...
              <div class="row">
                ...
                <div class="col-lg-12 ml-auto mt-1" style='color:black' data-aos="fade-up" data-aos-delay="100">
                  <div class="form-box">
                    <h3 class="h4 text-black mb-2">Daftar Pesanan</h3>
                        <form method='post' action='input_tracking.php' class="form-inline float-right mb-sm-2">
                            <input class="form-control-sm mr-sm-2" type="search" name='search' placeholder="Search">
                    ...
                    <table class="table table-bordered table-hover table-sm">
                        ...
                        <?php
                            if(isset($_POST["search"])){
                                $tracking->kd_pesanan = $_POST["search"];
                                if(!empty($tracking->kd_pesanan)){
                                    $rd = $tracking->search_read();
                                }
                                else{ $rd = $tracking->read_data(); }
                            }
                            else {
                                $rd = $tracking->read_data();
                            }
                            foreach($rd as $d){
                                echo "
                                <tr>
                                <td>$d[kd_pesanan]</td>
                                <td>$d[nama]</td>
                                <td>$d[jenis_doc] - $d[file]</td>
                                <td style='text-align:center'>
                                    <a href='form_tracking.php?kd_pesanan=$d[kd_pesanan]' class='btn-sm btn-secondary'>Input Tracking</a>
                                </td>
                                </tr>";
                            }
                        ?>
